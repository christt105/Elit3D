...
float4x4 Frustum::ComputeProjectionMatrix() const
{
	...
	MLOGE("Not all values of Frustum were initialized properly! Please initialize correctly before calling Frustum::ProjectionMatrix()!");
#endif
	return float4x4::nan;
}
...
bool Frustum::Contains(const vec &point) const
{
	...
	else
	{
		...
		MLOGE("Not all values of Frustum were initialized properly! Please initialize correctly before calling Frustum::Contains()!");
#endif
		return false;
	}
#endif
...
vec Frustum::UniformRandomPointInside(LCG &rng) const
{
	...
	else
	{
		...
		MLOGW("Rejection sampling failed in Frustum::UniformRandomPointInside! Producing a non-uniformly distributed point inside the frustum!");
		return FastRandomPointInside(rng);
	}
}
