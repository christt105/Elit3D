...
// #define BREAK_ON_ERROR_PRINTS - if an error message is printed (with MLOGE()), it is treated as if an assume() had failed, breaking to debugger.
// #define BREAK_ON_WARNING_PRINTS - if a warning message is printed (with MLOGW()), it is treated as if an assume() had failed, breaking to debugger.
...
#define assume_failed(message) do { \
		MLOGE("Assumption %s failed! in file %s, line %d!", message, __FILE__, __LINE__); \
		assert(false); \
	} while(0) 
