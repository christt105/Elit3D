...
OBB OBB::OptimalEnclosingOBB(const Polyhedron &convexHull)
{
	...
	*/
	...
	{
		// TODO
		MLOGW("Convex hull is degenerate and has only %d vertices/%d faces!", (int)convexHull.v.size(), (int)convexHull.f.size());
	...
	for(int i = 0; i < convexHull.NumFaces(); ++i) // O(F)
	{
		...
		{
			MLOGE("Input convex hull contains a degenerate face %d with only %d vertices! Cannot process this!",
	...
	for(size_t i = 0; i < faceNormals.size(); ++i)
	{
		...
		for(size_t j = 0; j < convexHull.v.size(); ++j)
		{
			...
			if (signedDistance > 1e-1f)
			{
				...
				MLOGE("Vertex %d is %f units deep on the wrong side of face %d (which has %d vertices and surface area %f): On the opposite side, extreme signed distance is vtx %d at d %f.", (int)j, signedDistance, 
...
#define IS_INTERNAL_EDGE(i) (reinterpret_cast<vec*>(&faceNormals[facesForEdge[i].first])->Dot(faceNormals[facesForEdge[i].second]) > 1.f - 1e-4f)
	...
	for(size_t i = 0; i < convexHull.v.size(); ++i)
		MLOGI("v[%d] = %s", (int)i, vec(convexHull.v[i]).ToString().c_str());
	for(size_t i = 0; i < edges.size(); ++i)
		MLOGI("e[%d] = %d->%d", (int)i, edges[i].first, edges[i].second);
#endif
	...
	for(size_t ii = 0; ii < spatialEdgeOrder.size(); ++ii)
	{
		...
		while(!traverseStack.empty()) // In amortized analysis, only a constant number of vertices are antipodal points for any edge?
		{
			...
			if (IsVertexAntipodalToEdge(convexHull, v, neighbors, f1a, f1b))
			{
				...
				{
					/* 
					MLOGE("Edge %d: %d->%d is antipodal to vertex %d, which is part of the same edge! This should be possible only if the input is degenerate planar!",
		...
		{
//			MLOGW("Due to numerical stability issues(?), could not find an antipodal vertex for edge %d! Check the scale of your input dataset! Good scale is having coordinates range e.g. [0.0,100.0]", edges[i]);
...
//			antipodalPointsForEdge[i].push_back(startingVertex);
	...
	for (size_t i = 0; i < antipodalPointsForEdge.size(); ++i)
	{
		...
		MLOGI("Antipodal points for edge %d: %s", (int)i, s.c_str());
	}
#endif
...
//							++numSmallerCompatibleEdgeIndices[edge];
	...
	for (size_t i = 0; i < edges.size(); ++i)
	{
		...
		MLOGI("Edge %d:%d->%d is compatible with: %s", (int)i, edges[i].first, edges[i].second, s.c_str());
	}
#endif
...
OBB OBB::Brute3EnclosingOBB(const Polyhedron &convexPolyhedron, const Quat &q)
{
	...
	while(nStepsNoProgress < nSteps + 100)
	{
		...
		if (volume < minVolume)
		{
//			MLOGI("Improved volume from %f to %f.", volume, minVolume);
			minOBB.axis[0] = d0;
			minOBB.axis[1] = d1;
			minOBB.axis[2] = d2;
			minOBB.r[0] = (dst[0] + dst[1]) * 0.5f;
